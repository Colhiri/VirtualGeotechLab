...
from bokeh.models import (CheckboxButtonGroup, CustomJS, IntEditor, TextInput, Button, ColumnDataSource, PointDrawTool,
                          Select, TableColumn, DataTable, NumberFormatter, TabPanel, Tabs)
...
class Graph_traxial:
    ...
    def run(self):
        ...
        self.GF_Cons = ConsolidationGraph(self.dct)
        self.tab_consolidation = self.GF_Cons.lay_consolidation

        self.lay_press = layout([[row(self.plot, column(self.interpolation_select, self.random_percent_min_text,
                                                        self.random_percent_max_text),
                                      column(self.name_new_shema,
                                      ...
                                      self.plot_volume,
                                      column(self.volume_interpolation_select, self.volume_random_percent_min_text,
                                             self.volume_random_percent_max_text)),
                                  ]], sizing_mode='fixed')

        self.tab_press = TabPanel(child=self.lay_press, title="Traxial_test")
        self.tab_consolidation = TabPanel(child=self.plot, title="Traxial_test1")
        self.tabs = Tabs(tabs=[self.tab_press, self.tab_consolidation])
...
if sys.argv[2] == 'traxial':
    ...
    graphs.run()
"""
from Graph_server import Graph_traxial as GF


class ConsolidationGraph(GF):
    def __init__(self, dct: dict):
        ...
        self.lay_consolidation = layout([[row(self.plot, column(self.name_new_shema, self.button_add_schema, self.button_delete_schema)),
                                  row(self.interpolation_select,
                                      self.schema_select,
                                      self.button_save_schema,
                                      self.button_reset_schema,
                                      self.count_points_min_text,
