...
def OCR_start(organise_dct: dict):
    ...
    g = 9.80665
    depth = 2
    OCR = 0.381226053639847
    effective_press = 0.522
    ...
    y1_last = 0.365 # press_spd_bekker[-1] * res.slope + res.intercept
    ...
    p1, p2, p3 = Point(press_spd[index_1], por_list[index_1]), Point(press_spd[index_2], por_list[index_2]), Point(
        (point_X_CASS),
        point_Y_CASS)
    l1 = Line(p1, p2)
    ...
    plt.plot(press_spd, por_list)
    # Нижняя касательная
    plt.plot(CAS_1_X, CAS_1_Y, color='black')

    # Верхняя касательная (необязательная)
    plt.plot(CAS_2_X, CAS_2_Y, color='black')
