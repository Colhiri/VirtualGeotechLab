...
class MechanicStart:
    ...
    def traxial_isp_run(self, row):

        mode_main_isp = 'CD'
        if self.organise_dct.get(f'F_traxial_CU'):
            mode_main_isp = 'CU'
        if self.organise_dct.get(f'F_traxial_UU'):
            mode_main_isp = 'UU'

        F = self.organise_dct.get(f'F_traxial_{mode_main_isp}')
        C = self.organise_dct.get(f'C_traxial_{mode_main_isp}')
        E0 = self.organise_dct.get(f'E_0')
        E50 = self.organise_dct.get(f'E_50')
        E_rzg = self.organise_dct.get(f'E_rzg')
        ...
        pressStarts, pressEnds = self.Traxial_press_value(mode=mode_main_isp)
        ...
        if not E50:
            ...
            pressStarts.insert(0, pressStarts[0] * K_0 if pressStarts[0] * K_0 > 0.05 else 0.05)
        ...
        values_for_Excel.update({'K0': (1 - math.sin(math.radians(F)))})
    ...
    def unaxial_sps_run(self, row):
        ...
        for name, pressStart, pressEnd in zip(namesISP, pressStarts, pressEnds):
            ...
            unaxial_run = SPS.Unaxial_isp(organise_dct=self.organise_dct,
                                          dct_combination=self.dct_combination,
                                          type_grunt_schemas=self.type_grunt_schemas)
            NewDF, values_for_Excel_right =  unaxial_run.main_isp()
    ...
    def OCR_isp_run(self, row):
        ...
        test_OCR = OCR_ISP.compression_OCR(self.organise_dct)
        test_OCR.aggeragation()
        values_for_Excel = test_OCR.create_values()
    ...
    def Traxial_press_value(self, mode='CD') -> (list[float | None], list[float | None]):
        """
